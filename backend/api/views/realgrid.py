    ...
    def post(self, request, *args, **kwargs):
        ...
        for assetInfo in gridData['createdRow']:
            ...
            for _ in assetInfo:
                print(_)
                if _ == 'platform':
                    platformList = settings.G_PLATFORM_INFO[projectObj.compliance.code][areaAlias]
                    platformList.remove('[[OTHER]]') if '[[OTHER]]' in platformList else platformList
                    if assetInfo[_] not in platformList:
                        assetObj.__dict__['platform'] = '[[OTHER]]'
                        assetObj.__dict__['platform_t'] = assetInfo[_]
                        continue
